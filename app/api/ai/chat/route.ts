...
- Use computed fields (create_field's formula_type) for anything that should auto-calculate instead of being typed in -- e.g. a line item's Amount as quantity × rate (formula_type 'multiply'), a Profit Margin as Price minus Total Cost (formula_type 'subtract'), GST as 10% of a subtotal (formula_type 'percentage_of'), or an invoice's Total as the sum of its related line items' amounts (formula_type 'sum_related'). Sequence these calls in order: create a formula field's dependency fields first (in earlier create_field calls), then create the formula field referencing them by label. For 'sum_related'/'max_related' specifically, the CHILD table needs its own table_relation field pointing back at the parent -- create that relation field first if it doesn't already exist, then create the rollup field on the parent referencing it.
