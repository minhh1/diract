...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ pageId: string; taskId: string; followUpId: string }> }) {
  ...
  const { data: remaining } = await admin.from("task_follow_ups").select("followed_up_at").eq("task_id", taskId).eq("is_done", true);
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ pageId: string; taskId: string; followUpId: string }> }) {
  const { pageId, taskId, followUpId } = await params;
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL!;
  const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY!;
  const admin = createClient(supabaseUrl, serviceRoleKey, { auth: { autoRefreshToken: false, persistSession: false } });
  ...
  const { data: entry, error } = await admin.from("task_follow_ups").update({ is_done: true }).eq("id", followUpId).eq("task_id", taskId).select().maybeSingle();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  if (!entry) return NextResponse.json({ error: "Follow-up not found" }, { status: 404 });

  const { data: doneEntries } = await admin.from("task_follow_ups").select("followed_up_at").eq("task_id", taskId).eq("is_done", true);
  const dates = (doneEntries || []).map((r: any) => String(r.followed_up_at).slice(0, 10));
  const latest = dates.length ? dates.reduce((a: string, b: string) => (a > b ? a : b)) : null;
  await admin.from("tasks").update({ awaiting_follow_up: true, follow_up_date: latest }).eq("id", taskId);

  await logTaskActivity(admin, { taskId, companyId: page.company_id, actorId: user.id, action: "follow_up_set", detail: "scheduled follow-up marked done" });
