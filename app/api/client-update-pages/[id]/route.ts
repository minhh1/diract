...
// matter name) + field defs. Value editing lives at .../values (PATCH),
// not here -- this is read-only.
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: groups }, { data: items }, { data: fields }] = await Promise.all([
    admin.from("client_update_groups").select("id, name, display_order").eq("page_id", id).order("display_order"),
    admin.from("client_update_page_items").select("id, project_id, group_id, display_order").eq("page_id", id).order("display_order"),
    admin.from("client_update_page_fields").select("id, field_source, field_key, label, display_order, client_visible").eq("page_id", id).order("display_order"),
  ]);
  ...
  const { data: customValues } = customFieldIds.length && projectIds.length
    ...
    : { data: [] as any[] };
  const customValueByKey = new Map((customValues || []).map((v: any) => [`${v.field_id}:${v.record_id}`, v]));
  ...
  const { data: adhocValues } = adhocFieldIds.length && itemIds.length
    ...
    : { data: [] as any[] };
  const adhocValueByKey = new Map((adhocValues || []).map((v: any) => [`${v.field_id}:${v.item_id}`, v.value_text]));
  ...
  return NextResponse.json({
    ...
    items: (items || []).map((i: any) => ({
      ...i,
      matterName: projectById.get(i.project_id)?.name || "",
