...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // A new top-level group starts on the shared column set (group_id IS NULL
  // -- see the fields route's header comment) rather than getting its own
  // copy immediately; it only diverges if someone explicitly customizes its
  // columns later (POST .../groups/[groupId]/customize-columns).
