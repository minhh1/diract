        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-10">
          <div>
            <div className="flex items-center gap-3 mb-4">
               <span className="px-3 py-1 bg-indigo-600 text-white rounded-full text-[9px] font-black uppercase tracking-widest italic shadow-lg shadow-indigo-100">
                 Niksen Time Pty Ltd
               </span>
            </div>
          ...
          <div className="flex items-center gap-4">
            {canShare && (
              <button onClick={handleShare} className="p-4 rounded-full bg-white border border-slate-100 text-slate-400 hover:text-black transition-all flex items-center gap-2 group">
              ...
              {activeTab === "checklist" && (
                <div className="max-w-4xl mx-auto space-y-2 py-4 animate-in fade-in zoom-in-95">
                  {tasks.length > 0 ? tasks.map(t => <TaskItem key={t.id} task={t} onRefresh={fetchDetails} />) : (
                    <div className="py-20 text-center text-slate-300 font-bold italic border-4 border-dashed border-slate-50 rounded-[48px]">Empty Checklist</div>
                  )}
                </div>
              )}
