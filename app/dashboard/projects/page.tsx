...
import { 
  Plus, ListChecks, Calendar as CalIcon, GanttChartSquare, 
  MapPin, Clock, Users, Loader2, Share2, ClipboardList, History
} from "lucide-react";
...
function ProjectContent() {
  ...
  if (!projectId) return (
    <div className="h-full flex flex-col items-center justify-center p-12 text-center bg-white font-sans antialiased">
      <div className="w-20 h-20 bg-slate-900 rounded-[32px] flex items-center justify-center shadow-2xl mb-8 rotate-3 transition-transform hover:rotate-0">
      ...
      <p className="max-w-md text-slate-400 font-medium mt-4 text-[15px]">
        {userProfile?.company_id ? `Active under Niksen Time Pty Ltd.` : "Independent workspace active. Create a project to begin."}
      </p>
    </div>
  );
                ...
                <div className="max-w-4xl mx-auto space-y-6 py-6">
                  {logs.map((log) => (
                    <div key={log.id} className="relative pl-12 group mb-6">
                      ...
                      <div className="absolute left-0 top-1 w-10 h-10 rounded-full bg-white border-2 border-indigo-500 flex items-center justify-center font-black text-[9px] text-indigo-600 uppercase italic shadow-sm">{log.profiles?.full_name?.substring(0, 2)}</div>
                      <div className="bg-slate-50 border border-slate-100 rounded-[28px] p-6 shadow-sm">
                        <p className="text-[14px] text-slate-600 font-medium"><span className="font-black text-slate-900">{log.profiles?.full_name}</span> <span className="mx-1 text-slate-400 italic">{log.action.toLowerCase()}</span> <span className="font-black text-indigo-600">"{log.details?.task_name}"</span></p>
