...
export default function PropertyDashboard({ propertyId, onBack }: PropertyDashboardProps) {
  ...
  const fetchAll = async () => {
    setLoading(true);
    const { data: p } = await supabase.from("properties").select(`*, holding_entity:holding_entity_id(name), council:council_entity_id(name), insurer:insurer_entity_id(name)`).eq("id", propertyId).single();
    const { data: l } = await supabase.from("audit_logs").select(`*, profiles:user_id(full_name)`).eq("property_id", propertyId).order('created_at', { ascending: false });
    if (p) setProperty(p); if (l) setLogs(l);
    setLoading(false);
  };
  ...
  const handleUpdate = async (field: string, newValue: any, displayName?: string) => {
    ...
    if (!error) {
      await supabase.from("audit_logs").insert([{
        ...
        action: `modified ${field.replace('_id', '')}`,
        ...
        <div className="col-span-2 p-5 flex items-center justify-between text-sm font-medium">
          {isEditing ? (
            ...
            <>
              ...
              <button onClick={() => { if(type==='relational') setPicker({isOpen:true, field, title:label, table: 'entities'}); else setEdit({field, value, type}); }} className="opacity-0 group-hover:opacity-100 text-indigo-600 p-2 hover:bg-white rounded-full transition-all border border-slate-100 shadow-sm">
  ...
  if (loading && !property) return <div className="p-20 text-center text-slate-300 font-medium uppercase tracking-widest animate-pulse">Syncing asset...</div>;
      ...
      <main className="flex-1 overflow-y-auto bg-[#F9FAFB] p-10">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="p-10 border-2 border-dashed border-red-100 bg-white rounded-[48px] flex items-center justify-between">
             <div>
                <p className="text-sm font-bold text-slate-900">Remove from portfolio</p>
                <p className="text-xs text-slate-400 mt-1">This marks the asset as deleted in all active trees.</p>
             </div>
             <DeleteAction table="properties" id={propertyId} identifier={property?.street_address} onRefresh={onBack} variant="button" />
          </div>
        </div>
      </main>
