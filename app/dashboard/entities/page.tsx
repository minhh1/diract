...
export default function EntityMasterPage() {
  ...
  const [tableCols, setTableCols] = useState<string[]>(['name', 'entity_type', 'abn', 'nab_connect_id']);
  ...
  const init = async () => {
    ...
    if (prefs) { 
      setTableCols(prefs.columns || ['name']); 
    ...
    <div className="flex flex-col h-screen bg-[#F9FAFB] font-sans antialiased text-slate-600 overflow-hidden">
      <header className="bg-white p-8 border-b border-slate-200 shrink-0">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-light text-slate-900 tracking-tight uppercase leading-none">Entities</h1>
        ...
        <div className="relative"><Search className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300" size={20} /><input placeholder="Search entities..." className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 pl-14 pr-8 text-sm font-medium outline-none focus:ring-8 focus:ring-black/5" value={search} onChange={e => setSearch(e.target.value)} /></div>
      </header>
                ...
                {tableCols.map((colId, idx) => (
                  <th key={colId} style={{ width: colWidths[colId] || 250, minWidth: '150px' }} className="relative border-r border-slate-100 group/header select-none p-0">
                    <div className="flex items-center h-full">
                      ...
                      }} className="p-4 cursor-move opacity-0 group-hover/header:opacity-100"><GripVertical size={14}/></div>
                      <div onClick={() => setSortConfig({ key: colId, direction: sortConfig.direction === 'asc' ? 'desc' : 'asc' })} className="flex-1 py-5 uppercase text-[10px] font-bold tracking-widest px-4 cursor-pointer">{colId}</div>
                    </div>
                  </th>
                ))}
