...
// DocumentPublicPageWidget on a dashboard (embedded=true, see
// components/dashboard/DocumentPublicPageWidget.tsx) instead of just
// linking out to it.
//
// GENUINELY UNAUTHENTICATED when reached via the standalone route -- unlike
// components/public/PublicTasksContent.tsx (which requires a signed-in
// session), this component itself makes no supabase.auth calls anywhere;
// access is gated purely server-side by the page being active + unexpired
// (+ an optional access code). Embedding it in a dashboard widget doesn't
// change that -- the viewer is already signed in to reach the dashboard at
// all, this component just doesn't care either way.
"use client";
...
interface Props {
  ...
  // standalone /public/documents/[pageId] route -- drops the
  // full-viewport background/padding (the widget's own card chrome already
  // supplies that) and skips hijacking the browser tab title.
  embedded?: boolean;
}

export default function PublicDocumentsContent({ pageId, embedded = false }: Props) {
  ...
  useEffect(() => {
    if (!embedded && data?.heading) document.title = data.heading;
  }, [data?.heading, embedded]);
