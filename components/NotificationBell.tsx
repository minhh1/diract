...
// Fixed floating icon + dropdown, pinned to the top-right corner of the
// viewport (mounted once in app/dashboard/layout.tsx, so it stays put
// across every /dashboard/* page -- there's no shared header to put it in
// otherwise, see that file). Open/close interaction mirrors components/
// Sidebar.tsx's own company-switcher dropdown idiom (self-contained state,
    ...
    <div className="fixed top-4 right-4 z-40" onClick={e => e.stopPropagation()}>
      <div className="relative">
        <button
          ...
          className="relative w-10 h-10 rounded-2xl flex items-center justify-center bg-white border border-slate-200 shadow-sm text-slate-400 hover:text-slate-700 hover:border-slate-300 transition-all"
        >
          <Bell size={17} />
          {unreadCount > 0 && (
        ...
        {open && (
          <div className="absolute top-full right-0 mt-2 w-80 bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden z-50">
