...
export default function MarketingNav({ secondaryLink }: { secondaryLink?: { href: string; label: string } }) {
  return (
    <nav className="fixed top-0 inset-x-0 z-50 bg-white/80 backdrop-blur border-b border-slate-100">
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-7">
          {secondaryLink && (
            <Link href={secondaryLink.href} className="text-[13px] text-slate-500 hover:text-indigo-600 transition-colors font-medium">
              {secondaryLink.label}
            </Link>
          )}
