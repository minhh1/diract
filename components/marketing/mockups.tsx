...
// app's actual component styling (see components/DataTable.tsx,
// components/MasterTable.tsx, components/dashboard/AutoTimeRecordingPanel.tsx)
// rather than an invented generic style -- same table chrome (bg-slate-50
// uppercase headers, border-slate-200/50 rows), same pill/badge tokens, and
// for the auto time recording drawer, the same layout down to its settings
// row order (date + scope, then description detail-level) and per-entry
// fields. All utility classes here are ones app/globals.css's `.dark`
...
// the same way the real app's dark mode works -- no separate dark palette
...
// Card chrome for the non-table widgets (charts, kanban, timeline,
// checklist) -- same rounded-2xl/border-slate-200/shadow-sm language as
// TableFrame so every mockup reads as one consistent design system, just
// without a real table equivalent to copy.
...
function Kanban({ columns }: { columns: { title: string; cards: string[] }[] }) {
  return (
    <div className="grid grid-cols-3 gap-2">
      {columns.map((col) => (
        <div key={col.title} className="rounded-xl bg-slate-50 p-2">
          <p className="text-[9px] font-semibold text-slate-400 uppercase tracking-widest mb-2 px-1">{col.title}</p>
          <div className="space-y-1.5">
            {col.cards.map((c) => (
              <div key={c} className="rounded-lg bg-white border border-slate-200 px-2 py-1.5 text-[10px] font-medium text-slate-600 shadow-sm">
                {c}
              </div>
            ))}
          </div>
      ...
      {steps.map((s, i) => (
        <div key={s.label} className="flex items-start gap-3">
          <div className="flex flex-col items-center">
            <span className={`w-3 h-3 rounded-full ${s.done ? "bg-indigo-500" : "bg-slate-200"}`} />
            {i < steps.length - 1 && <span className="w-px h-6 bg-slate-200" />}
          </div>
          <span className={`text-[12px] font-medium pb-3 ${s.done ? "text-slate-700" : "text-slate-400"}`}>{s.label}</span>
        </div>
      ))}
    </div>
  );
...
function MockTrustAccount() {
  return (
    <TableFrame
      ...
      rows={[
        ["Trust receipt — Smith Trust", { text: "+$45,000", badge: "emerald" }],
        ["Disbursement — Harbord Co.", { text: "-$1,200", badge: "rose" }],
        ["Balance", { text: "$43,800", badge: "indigo" }],
      ]}
    />
  );
}
...
function MockFeesReport() {
  return (
    <WidgetCard label="Fees report">
      <Bars values={[55, 70, 45, 90, 65]} color="bg-emerald-400" />
      <div className="mt-4">
        <Stat label="WIP outstanding" value="$84,200" />
      </div>
    </WidgetCard>
  );
}
...
function MockPrecedents() {
  return (
    <TableFrame
      label="Precedents"
      columns={["Precedent", "Practice area"]}
      rows={[
        ["Contract of Sale", { text: "Property", badge: "indigo" }],
        ["Deed of Trust", { text: "Trusts", badge: "violet" }],
        ["Lease Agreement", { text: "Property", badge: "indigo" }],
      ]}
    />
  );
}
...
function MockClientUpdates() {
  return (
    <WidgetCard label="Client update">
      <Timeline
        steps={[
          { label: "Contract signed", done: true },
          { label: "Finance approved", done: true },
          { label: "Settlement booked", done: false },
...
function MockTasks() {
  return (
    <WidgetCard label="Tasks">
      <Kanban
        columns={[
          { title: "To do", cards: ["Order title search"] },
          { title: "In progress", cards: ["Draft contract", "Review PPSR"] },
          { title: "Done", cards: ["Send engagement"] },
        ]}
      />
    </WidgetCard>
  );
