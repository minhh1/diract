...
export default function Sidebar() {
  ...
  const fetchInitialProjects = async () => {
    ...
    // Security: Fetch projects where user is a member (Corporate or Independent)
    const { data } = await supabase
      .from('projects')
      .select(`
        id, name, updated_at,
  ...
  const sortedProjects = [...treeProjects].sort((a, b) => {
    if (sortBy === "modified_desc") return new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime();
    if (sortBy === "address_alpha") return (a.properties?.suburb || "").localeCompare(b.properties?.suburb || "");
    return 0;
  });
  ...
  return (
    <div className="flex flex-col h-full bg-white px-4 py-8 border-r border-slate-200 overflow-hidden font-sans antialiased">
      ...
      <div className="flex items-center gap-3 px-3 mb-12 select-none">
        <div className="h-10 w-10 rounded-2xl bg-black flex items-center justify-center shadow-xl shadow-black/20">
      ...
      <nav className="flex-1 flex flex-col min-h-0">
        ...
        <div className="flex items-center justify-between px-5 mb-4 group">
          ...
          <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            {/* THIS BUTTON SPECIFICALLY TRIGGERS THE PROJECT MODAL */}
            <button 
              onClick={() => setIsProjectModalOpen(true)} 
              className="p-1 hover:bg-slate-100 rounded text-slate-400 hover:text-black transition-all" 
            ...
            <button 
              onClick={() => setTreeProjects(allProjects)} 
            ...
            <div className="relative">
              <SortAsc size={14} className="cursor-pointer text-slate-400 hover:text-black mt-0.5" />
              <select 
        ...
        <div className="flex-1 overflow-y-auto space-y-1 pr-2 custom-scrollbar">
          {sortedProjects.length > 0 ? (
            sortedProjects.map((proj) => {
              ...
              return (
                <div key={proj.id} className="group relative">
                  ...
                  <button 
                    onClick={() => setTreeProjects(treeProjects.filter(p => p.id !== proj.id))} 
            ...
            <div className="px-5 py-6 border-2 border-dashed border-slate-50 rounded-[32px] text-center">
              <p className="text-[11px] font-bold text-slate-300 italic">No projects added to your tree.</p>
            </div>
          )}
        </div>
      </nav>

      {/* 3. DYNAMIC USER FOOTER */}
      <div className="mt-auto pt-6 border-t border-slate-100 gap-2 flex flex-col">
        {userProfile ? (
          <div className="flex items-center gap-3 p-3.5 rounded-[28px] bg-slate-50 border border-slate-100 shadow-sm transition-all hover:border-slate-200">
            <div className="h-10 w-10 rounded-full bg-indigo-600 flex items-center justify-center text-[11px] font-black text-white border-2 border-white shadow-md shrink-0 italic">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <p className="text-[10px] font-bold text-indigo-500 uppercase tracking-tighter leading-none mt-1">
                {userProfile.company_id ? 'Niksen Staff' : 'Independent'}
              </p>
            </div>
          </div>
        ) : (
        ...
        <button 
          ...
          className="flex items-center gap-4 px-5 py-4 rounded-full text-[13px] font-bold text-slate-400 hover:text-red-600 hover:bg-red-50 transition-all group"
        >
      ...
      {/* PROJECT MODAL - ENSURE THIS IS NewProjectModal */}
      <NewProjectModal 
