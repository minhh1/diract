...
//
// Extracted from mobile/src/app/(app)/more/public/tasks/[pageId].tsx (still
// that screen's own implementation, just accepting pageId as a prop
// instead of reading it off the route) so the internal Tasks tab
// (mobile/src/app/(app)/tasks/index.tsx) can reuse the exact same
// experience for whichever task page represents "the whole company",
// rather than the generic raw-table RecordListView/RecordDetailView it
// used to fall back to -- see that file's own comment for why.
...
export function TaskPageContent({ pageId }: { pageId: string }) {
  const theme = useTheme();
