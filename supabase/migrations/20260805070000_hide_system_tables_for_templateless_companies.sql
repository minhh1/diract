-- A brand-new company (register_company_and_profile -- see
-- 20260805050000_fix_register_company_and_profile.sql, called by both the
-- generic signup at app/(marketing)/login/page.tsx and the law-firm-au
-- wizard) previously started with all 4 built-in tables (Projects/
-- Properties/Entities/Tasks) visible and empty, whether or not the company
-- ever installs a template. For a company that never installs one, those
-- tables have no purpose -- the user builds out whatever tables they
-- actually need via the schema editor or AI table builder instead. So new
-- companies now start with all 4 hidden (same shape/mechanism as a company
-- admin manually "deleting" one from Settings > Trash -- see
-- companies_disabled_system_tables.sql); nothing here touches existing
-- companies' current disabled_system_tables value.
--
-- Installing a template (install_company_template) must undo this for
-- whichever system tables that template actually populates, or a
-- template-driven signup (e.g. law-firm-au, which installs 'law-firm'
-- through app/auth/callback/route.ts moments after the company row is
-- created) would silently end up with its own Matters/Clients tables
-- hidden. Tasks isn't part of any template's system fields (there's
-- nothing a template or the marketplace could "give back" for it the way
-- there is for Projects/Properties/Entities), but every template install
-- means the company is now an active product user, so Tasks comes back
-- unconditionally alongside whichever business tables the template needs.
ALTER TABLE companies ALTER COLUMN disabled_system_tables SET DEFAULT jsonb_build_object(
  'projects',   jsonb_build_object('deleted_at', now(), 'field_ids', '[]'::jsonb),
  'properties', jsonb_build_object('deleted_at', now(), 'field_ids', '[]'::jsonb),
  'entities',   jsonb_build_object('deleted_at', now(), 'field_ids', '[]'::jsonb),
  'tasks',      jsonb_build_object('deleted_at', now(), 'field_ids', '[]'::jsonb)
);

CREATE OR REPLACE FUNCTION public.install_company_template(p_company_id uuid, p_template_id uuid, p_resolutions jsonb DEFAULT '{}'::jsonb, p_install_dashboards boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  -- New companies default to all 4 hidden; a template install is the
  -- signal that this company is now an active product user.
