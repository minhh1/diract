-- Locks down custom-table/custom-field schema mutation:
--   * A table or field that a template install actually created
--     (is_from_template = true) can never be edited or deleted again,
--     by anyone -- including a company admin. It stays fully usable
--     (records/values on it are untouched), just structurally frozen.
--   * Everything else (a company's own hand-added tables/fields, and any
--     field added later onto a template-sourced table) can be freely
--     added/edited/deleted by a company admin, same as today.
--   * A non-admin can no longer create or edit ANY table/field, template
--     or not -- this closes an existing gap where company_tables/
--     company_table_fields defaulted to "any member" for insert/update,
--     and company_custom_fields had no admin check at all (see
--     supabase/migrations/20260727040000_default_and_private_tables_dashboards.sql
--     and supabase/migrations/20260801400000_resource_permissions.sql for
--     the "before" policies this replaces).
-- Delete for a non-admin was already blocked (see supabase/archive_requests.sql's
-- prevent_non_admin_delete trigger); that's untouched here except for the
-- new is_from_template carve-out, which now blocks admins too.
--
-- Known limitation: company_table_fields (fields on a company's OWN custom
-- tables, as opposed to entities/projects/properties) never had per-row
-- template provenance tracked anywhere before this migration -- only the
-- PARENT company_tables row's provenance was recorded (company_template_table_map).
-- So the backfill below can mark a template-INSTALLED custom table itself
-- as is_from_template, but cannot retroactively tell which of that table's
-- EXISTING fields came bundled with the template vs. were added by an admin
-- afterwards -- those all default to is_from_template = false (editable),
-- which is the safe/non-destructive choice. Only NEW installs from this
-- point on get correct per-field locking, via install_company_template's
-- updated INSERT below.

-- ── 1. Provenance column ────────────────────────────────────────────────

ALTER TABLE company_tables ADD COLUMN IF NOT EXISTS is_from_template boolean NOT NULL DEFAULT false;
ALTER TABLE company_table_fields ADD COLUMN IF NOT EXISTS is_from_template boolean NOT NULL DEFAULT false;
ALTER TABLE company_custom_fields ADD COLUMN IF NOT EXISTS is_from_template boolean NOT NULL DEFAULT false;

-- Backfill from the two provenance mappings that already exist (company_table_fields
-- has no equivalent mapping to backfill from -- see header comment).
UPDATE company_tables SET is_from_template = true
WHERE id IN (
  SELECT installed_company_table_id FROM company_template_table_map
  WHERE resolution = 'created' AND installed_company_table_id IS NOT NULL
) AND is_from_template = false;

UPDATE company_custom_fields SET is_from_template = true
WHERE id IN (
  SELECT installed_company_custom_field_id FROM company_template_field_map
  WHERE resolution = 'created' AND installed_company_custom_field_id IS NOT NULL
) AND is_from_template = false;

-- ── 2. install_company_template: mark what it creates ───────────────────
-- NOTE: the live function had already drifted from supabase/
-- template_marketplace.sql's tracked body (gained a 4th p_install_dashboards
-- param, is_ledger/disable_record_dashboard on the table insert, more
-- column coverage on the field insert, and a formula-field-key resolution
-- pass) -- this replaces the REAL live 4-parameter signature, matching that
-- drifted body exactly, just adding is_from_template = true to all 3 INSERT
-- paths (tables, table fields, system fields) and tightening the entry
-- check from "any company member" to "company admin", matching the new
-- create/edit/delete rules -- a template install is itself a bulk "add
-- tables/fields" action. See the DROP FUNCTION just above this file's tail
-- if a stale 3-parameter overload from an earlier bad attempt still exists.
CREATE OR REPLACE FUNCTION install_company_template(
  p_company_id uuid,
  p_template_id uuid,
  p_resolutions jsonb DEFAULT '{}'::jsonb,
  p_install_dashboards boolean DEFAULT false
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE
  p_actor uuid := auth.uid();
  v_tbl RECORD;
  v_fld RECORD;
  v_sf RECORD;
  v_resolution text;
  v_existing_id uuid;
  v_new_table_id uuid;
  v_new_field_id uuid;
  v_new_slug text;
  v_new_key text;
  v_suffix int;
  v_linked_table_id uuid;
  v_target_table_id uuid;
  v_install_id uuid;
  v_overrides jsonb;
  v_apply_overrides boolean;
  v_tables_created int := 0;
  v_fields_created int := 0;
  v_dashboards_created int := 0;
  v_formula_table_id uuid;
  v_a_id uuid;
  v_b_id uuid;
  v_rel_id uuid;
BEGIN
  IF p_actor IS NULL OR NOT EXISTS (
    SELECT 1 FROM company_memberships WHERE company_id = p_company_id AND user_id = p_actor AND role = 'company_admin'
  ) THEN
    RAISE EXCEPTION 'only a company admin can install a template';
  END IF;

  IF EXISTS (SELECT 1 FROM company_template_installs WHERE company_id = p_company_id AND template_id = p_template_id) THEN
    RETURN jsonb_build_object('status', 'already_installed');
  END IF;

  CREATE TEMP TABLE IF NOT EXISTS tmp_template_table_map (source_id uuid PRIMARY KEY, installed_id uuid) ON COMMIT DROP;
  TRUNCATE tmp_template_table_map;

  -- Pass 1: tables
  FOR v_tbl IN SELECT * FROM template_definition_tables WHERE template_id = p_template_id ORDER BY display_order LOOP
    v_resolution := COALESCE(p_resolutions->'tables'->>v_tbl.slug, 'create_new');

    IF v_resolution = 'use_existing' THEN
      SELECT id INTO v_existing_id FROM company_tables WHERE company_id = p_company_id AND slug = v_tbl.slug AND deleted_at IS NULL LIMIT 1;
      IF v_existing_id IS NULL THEN
        RAISE EXCEPTION 'use_existing chosen for table % but no matching table exists', v_tbl.slug;
      END IF;
      INSERT INTO tmp_template_table_map (source_id, installed_id) VALUES (v_tbl.id, v_existing_id);
      INSERT INTO company_template_table_map (company_id, template_id, source_template_table_id, installed_company_table_id, resolution)
        VALUES (p_company_id, p_template_id, v_tbl.id, v_existing_id, 'used_existing');
    ELSE
      v_new_slug := v_tbl.slug;
      v_suffix := 1;
      WHILE EXISTS (SELECT 1 FROM company_tables WHERE company_id = p_company_id AND slug = v_new_slug AND deleted_at IS NULL) LOOP
        v_suffix := v_suffix + 1;
        v_new_slug := v_tbl.slug || '-' || v_suffix;
      END LOOP;

      INSERT INTO company_tables (company_id, name, slug, icon, color, primary_field_key, display_order, is_ledger, disable_record_dashboard, is_from_template)
        VALUES (p_company_id, v_tbl.name, v_new_slug, v_tbl.icon, v_tbl.color, v_tbl.primary_field_key, v_tbl.display_order, v_tbl.is_ledger, v_tbl.disable_record_dashboard, true)
        RETURNING id INTO v_new_table_id;
      v_tables_created := v_tables_created + 1;

      INSERT INTO tmp_template_table_map (source_id, installed_id) VALUES (v_tbl.id, v_new_table_id);
      INSERT INTO company_template_table_map (company_id, template_id, source_template_table_id, installed_company_table_id, resolution)
        VALUES (p_company_id, p_template_id, v_tbl.id, v_new_table_id, 'created');

      INSERT INTO schema_change_log (company_id, actor_id, entity_type, entity_id, entity_label, action, after)
        VALUES (p_company_id, p_actor, 'company_table', v_new_table_id, v_tbl.name, 'create',
          jsonb_build_object('name', v_tbl.name, 'slug', v_new_slug, 'from_template', p_template_id));
    END IF;
  END LOOP;

  -- Pass 2: fields, only for tables actually created this install
  FOR v_tbl IN SELECT * FROM template_definition_tables WHERE template_id = p_template_id LOOP
    SELECT installed_id INTO v_target_table_id FROM tmp_template_table_map WHERE source_id = v_tbl.id;
    SELECT resolution INTO v_resolution FROM company_template_table_map
      WHERE company_id = p_company_id AND template_id = p_template_id AND source_template_table_id = v_tbl.id;

    IF v_resolution = 'created' THEN
      FOR v_fld IN SELECT * FROM template_definition_table_fields WHERE template_table_id = v_tbl.id ORDER BY display_order LOOP
        v_linked_table_id := NULL;
        IF v_fld.linked_template_table_id IS NOT NULL THEN
          SELECT installed_id INTO v_linked_table_id FROM tmp_template_table_map WHERE source_id = v_fld.linked_template_table_id;
        END IF;

        INSERT INTO company_table_fields (
          company_id, table_id, field_key, label, field_type, select_options,
          linked_table_id, linked_system_table, linked_display_field,
          is_required, is_unique, show_in_table, display_order, section_name, help_text,
          auto_number_prefix, auto_number_start, auto_number_pad,
          default_value, linked_filter_column, linked_filter_value,
          is_from_template
        ) VALUES (
          p_company_id, v_target_table_id, v_fld.field_key, v_fld.label, v_fld.field_type, v_fld.select_options,
          v_linked_table_id, v_fld.linked_system_table, v_fld.linked_display_field,
          v_fld.is_required, v_fld.is_unique, v_fld.show_in_table, v_fld.display_order, v_fld.section_name, v_fld.help_text,
          v_fld.auto_number_prefix, v_fld.auto_number_start, v_fld.auto_number_pad,
          v_fld.default_value, v_fld.linked_filter_column, v_fld.linked_filter_value,
          true
        );
      END LOOP;
    END IF;
  END LOOP;

  -- Pass 3: resolve formula field keys to installed field ids (created tables only).
  FOR v_tbl IN SELECT * FROM template_definition_tables WHERE template_id = p_template_id LOOP
    SELECT installed_id INTO v_target_table_id FROM tmp_template_table_map WHERE source_id = v_tbl.id;
    SELECT resolution INTO v_resolution FROM company_template_table_map
      WHERE company_id = p_company_id AND template_id = p_template_id AND source_template_table_id = v_tbl.id;
    IF v_resolution IS DISTINCT FROM 'created' THEN CONTINUE; END IF;

    FOR v_fld IN SELECT * FROM template_definition_table_fields
      WHERE template_table_id = v_tbl.id AND formula_type IS NOT NULL LOOP

      IF v_fld.formula_type = 'sum_related' THEN
        SELECT m.installed_id INTO v_formula_table_id
          FROM template_definition_tables tt JOIN tmp_template_table_map m ON m.source_id = tt.id
          WHERE tt.template_id = p_template_id AND tt.slug = v_fld.formula_related_table_slug;
      ELSE
        v_formula_table_id := v_target_table_id;
      END IF;
      IF v_formula_table_id IS NULL THEN CONTINUE; END IF;

      v_a_id := NULL; v_b_id := NULL; v_rel_id := NULL;
      SELECT id INTO v_a_id FROM company_table_fields
        WHERE table_id = v_formula_table_id AND field_key = v_fld.formula_field_a_key AND deleted_at IS NULL;
      IF v_fld.formula_field_b_key IS NOT NULL THEN
        SELECT id INTO v_b_id FROM company_table_fields
          WHERE table_id = v_formula_table_id AND field_key = v_fld.formula_field_b_key AND deleted_at IS NULL;
      END IF;
      IF v_fld.formula_relation_field_key IS NOT NULL THEN
        SELECT id INTO v_rel_id FROM company_table_fields
          WHERE table_id = v_formula_table_id AND field_key = v_fld.formula_relation_field_key AND deleted_at IS NULL;
      END IF;

      UPDATE company_table_fields SET
        formula_type = v_fld.formula_type,
        formula_field_a_id = v_a_id,
        formula_field_b_id = v_b_id,
        formula_percent = v_fld.formula_percent,
        formula_relation_field_id = v_rel_id
      WHERE table_id = v_target_table_id AND field_key = v_fld.field_key AND deleted_at IS NULL;
    END LOOP;
  END LOOP;

  -- Pass 4: dashboards (see install_template_dashboards) -- only when the
  -- user opted in.
  IF p_install_dashboards THEN
    SELECT install_template_dashboards(p_company_id, p_template_id) INTO v_dashboards_created;
  END IF;

  -- System fields (entities/projects/properties)
  FOR v_sf IN SELECT * FROM template_definition_system_fields WHERE template_id = p_template_id ORDER BY display_order LOOP
    v_resolution := COALESCE(p_resolutions->'systemFields'->>(v_sf.table_name || ':' || v_sf.field_key), 'create_new');

    IF v_resolution = 'use_existing' THEN
      SELECT id INTO v_existing_id FROM company_custom_fields
        WHERE company_id = p_company_id AND table_name = v_sf.table_name AND field_key = v_sf.field_key AND deleted_at IS NULL LIMIT 1;
      IF v_existing_id IS NULL THEN
        RAISE EXCEPTION 'use_existing chosen for field %:% but no matching field exists', v_sf.table_name, v_sf.field_key;
      END IF;
      INSERT INTO company_template_field_map (company_id, template_id, source_template_system_field_id, target_table_name, installed_company_custom_field_id, resolution)
        VALUES (p_company_id, p_template_id, v_sf.id, v_sf.table_name, v_existing_id, 'used_existing');
    ELSE
      v_new_key := v_sf.field_key;
      v_suffix := 1;
      WHILE EXISTS (SELECT 1 FROM company_custom_fields WHERE company_id = p_company_id AND table_name = v_sf.table_name AND field_key = v_new_key AND deleted_at IS NULL) LOOP
        v_suffix := v_suffix + 1;
        v_new_key := v_sf.field_key || '_' || v_suffix;
      END LOOP;

      INSERT INTO company_custom_fields (
        company_id, table_name, field_key, label, field_type, select_options,
        is_required, is_unique, display_order, section_name, help_text, default_value,
        auto_generate, auto_generate_type, auto_generate_prefix,
        linked_table, linked_display_column, grid_width, show_in_table,
        is_from_template
      ) VALUES (
        p_company_id, v_sf.table_name, v_new_key, v_sf.label, v_sf.field_type, v_sf.select_options,
        v_sf.is_required, v_sf.is_unique, v_sf.display_order, v_sf.section_name, v_sf.help_text, v_sf.default_value,
        v_sf.auto_generate, v_sf.auto_generate_type, v_sf.auto_generate_prefix,
        v_sf.linked_table, v_sf.linked_display_column, 2, false,
        true
      ) RETURNING id INTO v_new_field_id;
      v_fields_created := v_fields_created + 1;

      INSERT INTO company_template_field_map (company_id, template_id, source_template_system_field_id, target_table_name, installed_company_custom_field_id, resolution)
        VALUES (p_company_id, p_template_id, v_sf.id, v_sf.table_name, v_new_field_id, 'created');

      INSERT INTO schema_change_log (company_id, actor_id, entity_type, entity_id, entity_label, action, after)
        VALUES (p_company_id, p_actor, 'company_custom_field', v_new_field_id, v_sf.label, 'create',
          jsonb_build_object('table_name', v_sf.table_name, 'field_key', v_new_key, 'from_template', p_template_id));
    END IF;
  END LOOP;

  v_apply_overrides := COALESCE((p_resolutions->>'applyLabelOverrides')::boolean, false);
  IF v_apply_overrides THEN
    SELECT suggested_label_overrides INTO v_overrides FROM template_definitions WHERE id = p_template_id;
    IF v_overrides IS NOT NULL AND v_overrides <> '{}'::jsonb THEN
      UPDATE companies SET table_label_overrides = table_label_overrides || v_overrides WHERE id = p_company_id;
    END IF;
  END IF;

  INSERT INTO company_template_installs (company_id, template_id, installed_by, label_overrides_applied)
    VALUES (p_company_id, p_template_id, p_actor, v_apply_overrides)
    RETURNING id INTO v_install_id;

  INSERT INTO schema_change_log (company_id, actor_id, entity_type, entity_id, entity_label, action, after)
    VALUES (p_company_id, p_actor, 'company_template_install', v_install_id,
      (SELECT name FROM template_definitions WHERE id = p_template_id), 'create',
      jsonb_build_object('template_id', p_template_id));

  RETURN jsonb_build_object(
    'status', 'installed', 'install_id', v_install_id,
    'tables_created', v_tables_created, 'fields_created', v_fields_created, 'dashboards_created', v_dashboards_created
  );
END;
$$;

-- Drop the stale 3-parameter overload this migration mistakenly created on
-- its first pass (before the live 4-parameter drift above was discovered) --
-- harmless dead code, but two overloads risk PostgREST ambiguous-function
-- errors on RPC calls.
DROP FUNCTION IF EXISTS install_company_template(uuid, uuid, jsonb);

-- ── 3. uninstall_company_template: admin-only, and the sanctioned bypass
-- of the new template lock (it's specifically removing what it created --
-- see the trigger in step 5). app.bypass_template_lock is transaction-local
-- (SET LOCAL), so it can never leak into any other statement/session.
-- NOTE: same live-drift situation as install_company_template above -- the
-- live body already removed installed dashboards too (company_template_
-- dashboard_map, paired with install's Pass 4 / install_template_dashboards),
-- which supabase/template_marketplace.sql's tracked copy doesn't show. That
-- pass is restored here alongside the admin-only tightening.
CREATE OR REPLACE FUNCTION uninstall_company_template(
  p_company_id uuid,
  p_template_id uuid
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE
  p_actor uuid := auth.uid();
  v_install company_template_installs%ROWTYPE;
  v_overrides jsonb;
  v_key text;
  v_tables_removed int := 0;
  v_fields_removed int := 0;
  v_dashboards_removed int := 0;
BEGIN
  IF p_actor IS NULL OR NOT EXISTS (
    SELECT 1 FROM company_memberships WHERE company_id = p_company_id AND user_id = p_actor AND role = 'company_admin'
  ) THEN
    RAISE EXCEPTION 'only a company admin can uninstall a template';
  END IF;

  SELECT * INTO v_install FROM company_template_installs WHERE company_id = p_company_id AND template_id = p_template_id;
  IF NOT FOUND THEN
    RETURN jsonb_build_object('status', 'not_installed');
  END IF;

  PERFORM set_config('app.bypass_template_lock', 'on', true);

  UPDATE company_tables SET deleted_at = now() WHERE id IN (
    SELECT installed_company_table_id FROM company_template_table_map
    WHERE company_id = p_company_id AND template_id = p_template_id AND resolution = 'created'
  ) AND deleted_at IS NULL;
  GET DIAGNOSTICS v_tables_removed = ROW_COUNT;

  UPDATE company_custom_fields SET deleted_at = now() WHERE id IN (
    SELECT installed_company_custom_field_id FROM company_template_field_map
    WHERE company_id = p_company_id AND template_id = p_template_id AND resolution = 'created'
  ) AND deleted_at IS NULL;
  GET DIAGNOSTICS v_fields_removed = ROW_COUNT;

  UPDATE company_dashboards SET deleted_at = now() WHERE id IN (
    SELECT installed_company_dashboard_id FROM company_template_dashboard_map
    WHERE company_id = p_company_id AND template_id = p_template_id
  ) AND deleted_at IS NULL;
  GET DIAGNOSTICS v_dashboards_removed = ROW_COUNT;

  IF v_install.label_overrides_applied THEN
    SELECT suggested_label_overrides INTO v_overrides FROM template_definitions WHERE id = p_template_id;
    IF v_overrides IS NOT NULL THEN
      FOR v_key IN SELECT jsonb_object_keys(v_overrides) LOOP
        UPDATE companies SET table_label_overrides = table_label_overrides - v_key WHERE id = p_company_id;
      END LOOP;
    END IF;
  END IF;

  DELETE FROM company_template_table_map WHERE company_id = p_company_id AND template_id = p_template_id;
  DELETE FROM company_template_field_map WHERE company_id = p_company_id AND template_id = p_template_id;
  DELETE FROM company_template_dashboard_map WHERE company_id = p_company_id AND template_id = p_template_id;

  INSERT INTO schema_change_log (company_id, actor_id, entity_type, entity_id, entity_label, action, before)
    VALUES (p_company_id, p_actor, 'company_template_install', v_install.id,
      (SELECT name FROM template_definitions WHERE id = p_template_id), 'delete',
      to_jsonb(v_install));

  DELETE FROM company_template_installs WHERE id = v_install.id;

  RETURN jsonb_build_object(
    'status', 'uninstalled', 'tables_removed', v_tables_removed,
    'fields_removed', v_fields_removed, 'dashboards_removed', v_dashboards_removed
  );
END;
$$;

-- ── 4. RLS: admin-only create/edit, template rows frozen ────────────────
-- company_tables -- private (owner_user_id = self) tables keep working
-- exactly as before (a separate, unrelated feature to this rule); the
-- "any member can touch a shared table with no explicit grant" branch is
-- removed from INSERT/UPDATE. Resource-level editor/admin grants
-- (resource_permissions, a finer opt-in layer companies can already turn
-- on) are left intact for UPDATE/DELETE. NOT is_from_template applies
-- regardless of role, so even is_current_user_admin() can't get through
-- on a template-sourced row.
DROP POLICY IF EXISTS ct_insert ON company_tables;
CREATE POLICY ct_insert ON company_tables
  FOR INSERT WITH CHECK (
    company_id = active_company_id()
    AND (owner_user_id IS NULL OR owner_user_id = auth.uid())
    AND (is_default = false OR is_current_user_admin())
    AND (owner_user_id = auth.uid() OR is_current_user_admin())
  );

DROP POLICY IF EXISTS ct_update ON company_tables;
CREATE POLICY ct_update ON company_tables
  FOR UPDATE
  USING (
    company_id = active_company_id()
    AND NOT is_from_template
    AND (
      owner_user_id = auth.uid()
      OR is_current_user_admin()
      OR user_resource_role('table', id) = ANY (ARRAY['editor', 'admin'])
    )
  )
  WITH CHECK (
    company_id = active_company_id()
    AND NOT is_from_template
    AND (
      owner_user_id = auth.uid()
      OR is_current_user_admin()
      OR user_resource_role('table', id) = ANY (ARRAY['editor', 'admin'])
    )
    AND (is_default = false OR is_current_user_admin())
  );

DROP POLICY IF EXISTS ct_delete ON company_tables;
CREATE POLICY ct_delete ON company_tables
  FOR DELETE
  USING (
    company_id = active_company_id()
    AND NOT is_from_template
    AND (owner_user_id = auth.uid() OR is_current_user_admin() OR user_resource_role('table', id) = 'admin')
  );

-- company_table_fields -- same shape, joined back to the parent table for
-- the owner/admin/resource-role check (a field has no owner of its own),
-- but is_from_template is checked on the FIELD row itself, not the table --
-- a template-sourced table can still gain new, freely-editable fields
-- added later by an admin.
DROP POLICY IF EXISTS ctf_insert ON company_table_fields;
CREATE POLICY ctf_insert ON company_table_fields
  FOR INSERT WITH CHECK (
    company_id = active_company_id()
    AND EXISTS (
      SELECT 1 FROM company_tables ct
      WHERE ct.id = company_table_fields.table_id
        AND (
          ct.owner_user_id = auth.uid()
          OR is_current_user_admin()
          OR user_resource_role('table', ct.id) = ANY (ARRAY['editor', 'admin'])
        )
    )
  );

DROP POLICY IF EXISTS ctf_update ON company_table_fields;
CREATE POLICY ctf_update ON company_table_fields
  FOR UPDATE
  USING (
    company_id = active_company_id()
    AND NOT is_from_template
    AND EXISTS (
      SELECT 1 FROM company_tables ct
      WHERE ct.id = company_table_fields.table_id
        AND (
          ct.owner_user_id = auth.uid()
          OR is_current_user_admin()
          OR user_resource_role('table', ct.id) = ANY (ARRAY['editor', 'admin'])
        )
    )
  )
  WITH CHECK (
    company_id = active_company_id()
    AND NOT is_from_template
    AND EXISTS (
      SELECT 1 FROM company_tables ct
      WHERE ct.id = company_table_fields.table_id
        AND (
          ct.owner_user_id = auth.uid()
          OR is_current_user_admin()
          OR user_resource_role('table', ct.id) = ANY (ARRAY['editor', 'admin'])
        )
    )
  );

DROP POLICY IF EXISTS ctf_delete ON company_table_fields;
CREATE POLICY ctf_delete ON company_table_fields
  FOR DELETE
  USING (
    company_id = active_company_id()
    AND NOT is_from_template
    AND EXISTS (
      SELECT 1 FROM company_tables ct
      WHERE ct.id = company_table_fields.table_id
        AND (ct.owner_user_id = auth.uid() OR is_current_user_admin() OR user_resource_role('table', ct.id) = 'admin')
    )
  );

-- company_custom_fields -- previously had NO admin/owner gate at all on
-- insert/update/delete (just company_id match); this is the biggest single
-- tightening in this migration; entities/projects/properties custom fields
-- have no per-table owner/resource-permission concept, so it's a flat
-- admin check.
DROP POLICY IF EXISTS ccf_insert ON company_custom_fields;
CREATE POLICY ccf_insert ON company_custom_fields
  FOR INSERT WITH CHECK (
    company_id = active_company_id()
    AND is_current_user_admin()
  );

DROP POLICY IF EXISTS ccf_update ON company_custom_fields;
CREATE POLICY ccf_update ON company_custom_fields
  FOR UPDATE
  USING (company_id = active_company_id() AND NOT is_from_template AND is_current_user_admin())
  WITH CHECK (company_id = active_company_id() AND NOT is_from_template AND is_current_user_admin());

DROP POLICY IF EXISTS ccf_delete ON company_custom_fields;
CREATE POLICY ccf_delete ON company_custom_fields
  FOR DELETE
  USING (company_id = active_company_id() AND NOT is_from_template AND is_current_user_admin());

-- ── 5. Delete trigger: template lock applies even to an admin ───────────
-- Extends supabase/archive_requests.sql's prevent_non_admin_delete with a
-- check that runs BEFORE the existing admin/service_role bypass, so a
-- template-sourced row on these 3 tables can't be deleted or archived by
-- anyone -- except uninstall_company_template's own soft-deletes above,
-- which set app.bypass_template_lock for the duration of their transaction.
CREATE OR REPLACE FUNCTION prevent_non_admin_delete()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  IF TG_TABLE_NAME IN ('company_tables', 'company_table_fields', 'company_custom_fields')
     AND COALESCE(OLD.is_from_template, false)
     AND current_setting('app.bypass_template_lock', true) IS DISTINCT FROM 'on'
  THEN
    IF TG_OP = 'DELETE' THEN
      RAISE EXCEPTION 'This was installed from a template and can never be deleted.' USING ERRCODE = '42501';
    ELSIF NEW.deleted_at IS DISTINCT FROM OLD.deleted_at THEN
      RAISE EXCEPTION 'This was installed from a template and can never be archived or deleted.' USING ERRCODE = '42501';
    END IF;
  END IF;

  IF auth.role() = 'service_role' OR is_current_user_admin() THEN
    RETURN COALESCE(NEW, OLD);
  END IF;

  IF TG_OP = 'DELETE' THEN
    RAISE EXCEPTION 'Only a company admin can permanently delete this record.' USING ERRCODE = '42501';
  END IF;

  IF NEW.deleted_at IS DISTINCT FROM OLD.deleted_at THEN
    RAISE EXCEPTION 'Only a company admin can archive this record directly. Submit an archive request instead.' USING ERRCODE = '42501';
  END IF;

  RETURN NEW;
END;
$$;

-- ── 6. Archive requests: don't let a non-admin submit a doomed request
-- for something that can never be deleted anyway.
DROP POLICY IF EXISTS archive_requests_insert ON archive_requests;
CREATE POLICY archive_requests_insert ON archive_requests
  FOR INSERT WITH CHECK (
    company_id = active_company_id()
    AND requested_by = auth.uid()
    AND status = 'pending'
    AND NOT (entity_table = 'company_tables' AND EXISTS (SELECT 1 FROM company_tables WHERE id = entity_id AND is_from_template))
    AND NOT (entity_table = 'company_table_fields' AND EXISTS (SELECT 1 FROM company_table_fields WHERE id = entity_id AND is_from_template))
    AND NOT (entity_table = 'company_custom_fields' AND EXISTS (SELECT 1 FROM company_custom_fields WHERE id = entity_id AND is_from_template))
  );
