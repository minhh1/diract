...
-- (concurrent work, unrelated to the Irregularities engine) made
-- client_update_page_items.record_table/record_id NOT NULL, generalizing
-- away from the old project_id/entity_id/custom_record_id triple.
-- auto_fed_upsert_item still only set custom_record_id -- fine while the
-- new columns didn't exist, but the moment they became NOT NULL, opening a
-- brand new auto-fed item (any rule violation on a record that has never
-- been flagged before, or whose only prior item is already Resolved --
-- the "find existing" lookup only matches an Open item, so a fixed-then-
-- regressed record is "new" again) started failing with a NOT NULL
-- violation. Found live: fixing then un-fixing "Missing GST Report
-- Frequency" on a real entity threw exactly this error re-opening it.
--
-- record_table for an auto-fed item is the same value
-- 20260729210000's own backfill used for a custom_table page:
-- source_table_id::text (here, the registry's target_table_id, which is
-- the same company_tables.id) -- record_id is the same id as
-- custom_record_id.
