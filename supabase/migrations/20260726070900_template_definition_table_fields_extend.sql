-- template_definition_table_fields (the marketplace catalog's field shape)
-- is missing default_value/linked_filter_column/linked_filter_value, even
-- though company_table_fields (what a real installed copy uses) has all
-- three -- confirmed via install_company_template's field-copy INSERT list,
-- which simply never passes them through. This has been a silent gap since
-- those columns were added to company_table_fields (default_value,
-- $current_user filtering): a NEW install of any template using them starts
-- without the default/filter a manually-seeded company already has. Closing
-- it here because this pass is about to make both columns load-bearing for
-- new installs too (Disbursements' quantity default_value, Staff's
-- role/team-scoped filter) -- without this, a brand-new company installing
-- the Law Firm template would regress on the exact bugs being fixed today.
ALTER TABLE template_definition_table_fields
  ADD COLUMN IF NOT EXISTS default_value text,
  ADD COLUMN IF NOT EXISTS linked_filter_column text,
  ADD COLUMN IF NOT EXISTS linked_filter_value text;

-- Thread the three new columns through the field-copy pass of
-- install_company_template -- otherwise the columns above would exist on
-- the catalog but never actually reach a newly-installed company's copy.
-- Everything else in this function body is unchanged from
-- supabase/template_marketplace_formula_ledger.sql (the last migration that
-- touched it).
...
BEGIN
  ...
  FOR v_tbl IN SELECT * FROM template_definition_tables WHERE template_id = p_template_id ORDER BY display_order LOOP
    ...
    ELSE
      ...
      INSERT INTO company_tables (company_id, name, slug, icon, color, primary_field_key, display_order, is_ledger, disable_record_dashboard)
        VALUES (p_company_id, v_tbl.name, v_new_slug, v_tbl.icon, v_tbl.color, v_tbl.primary_field_key, v_tbl.display_order, v_tbl.is_ledger, v_tbl.disable_record_dashboard)
