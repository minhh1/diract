-- Carries company_tables.disable_record_dashboard (see
-- company_tables_disable_record_dashboard.sql) through template
-- installation, the same way template_marketplace_formula_ledger.sql did
-- for is_ledger: a template_definition_tables column, copied onto the
-- installed company_tables row in install_company_template's Pass 1. Full
-- redeclaration of install_company_template (same approach every prior
-- migration to this function has used) -- only Pass 1's INSERT/VALUES
-- changes; everything else carried over unchanged from
-- template_marketplace_dashboards_install.sql.

ALTER TABLE template_definition_tables ADD COLUMN IF NOT EXISTS disable_record_dashboard boolean NOT NULL DEFAULT false;
