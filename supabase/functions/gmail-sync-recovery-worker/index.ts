...
// Return shape carries the failure reason -- see the matching fix/comment
// in gmail-migration-worker's importMessage. Neither failure branch here
// nor the caller previously logged anything, unlike label_sync's
// apply_label_failed, so a message that consistently fails to import was
        ...
        if (msgIds.length) {
          ...
          for (const msgId of msgIds) {
            ...
            if (importResult.ok) {
              ...
              // transient. See the matching fix/comment in
              // gmail-migration-worker's email_sync branch, which is where
              // this was actually diagnosed live.
